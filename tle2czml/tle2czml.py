...
def create_satellite_packet(
    sat, sim_start_time, sim_end_time, propagation_time_step_seconds
):
    ...
    packet.position = create_position(
        sim_start_time, sim_end_time, propagation_time_step_seconds, sat.tle_object
    )
    return packet
...
def create_path(total_path_interval, sat, sim_start_time, sim_end_time):
    ...
    sub_path_interval_end = sub_path_interval_start + timedelta(
        minutes=left_over_minutes
    )
    sub_path_interval_str = (
    ...
    for _ in range(number_of_full_orbits + 1):
        lead_or_trail_times.append(
            {
                ...
                "number": [0, orbital_time_in_seconds, orbital_time_in_seconds, 0],
            }
        )
        ...
        sub_path_interval_end = sub_path_interval_start + timedelta(
            minutes=sat.orbital_time_in_minutes
        )
        sub_path_interval_str = (
            sub_path_interval_start.isoformat()
            + "/"
        ...
        lead_or_trail_times.append(
            {
                "interval": sub_path_interval_str,
                "epoch": sub_path_interval_start.isoformat(),
                "number": [0, 0, orbital_time_in_seconds, orbital_time_in_seconds],
            }
        )
...
def get_future_sat_positions(
    sat_tle, number_of_positions, start_time, propagation_time_step_seconds
):
    ...
    for _ in range(number_of_positions):
        ...
        eci_position, _ = sat_tle.propagate(
            current_time.year,
            current_time.month,
            current_time.day,
            current_time.hour,
...
def tles_to_czml(
    ...
    for sat in satellite_array:
        ...
        sat_packet = create_satellite_packet(
            sat, start_time, end_time, propagation_time_step_seconds
        )
